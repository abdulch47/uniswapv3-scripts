...
// Pool addresses
USDT_WETH_500 = '0x404F0f1EDC16b85E1239F96358C028d193C5c800'
...
async function main() {
  ...
  await usdtContract.connect(signer2).approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther('100000'))
  // await usdcContract.connect(signer2).approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther('1000'))

  const poolContract = new Contract(USDT_WETH_500, artifacts.UniswapV3Pool.abi, provider)

  const poolData = await getPoolData(poolContract)

  const UsdtToken = new Token(31337, TETHER_ADDRESS, 18, 'USDT', 'Tether')
  const WETHToken = new Token(31337, WETH_ADDRESS, 18, 'WETH', 'Wrapped ETH')
  ...
  const { amount0: amount0Desired} = position.mintAmounts
